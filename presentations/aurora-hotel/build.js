...
/* =========================================================
   04 · ПОСАДКА · ВИД СВЕРХУ
   ========================================================= */
{
  const s = pres.addSlide();
  plate(s, A("aerial.jpg"));
  labels(s, { left: "Посадка на участок", right: "Вид сверху", color: INK, rightColor: INK });
  logo(s);
  pageNo(s, INK);
  s.addNotes(
    "Тот же генплан объёмом — читается за пять секунд там, где чертёж требует объяснений.\n" +
    "Здесь видно то, что на чертеже не считывается: кольцевой проезд замкнут, стоянки лежат по периметру " +
    "вне пятна застройки, зелёный клин с рядовой посадкой отделяет корпус от улицы."
  );
}
