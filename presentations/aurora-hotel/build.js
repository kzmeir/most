   ...
   ========================================================= */
{
  const s = pres.addSlide();
  s.background = { color: WHITE };
  labels(s, { left: "Архитектура", sub: "Генезис формы", right: "Концепция" });

  blockTitle(s, CX, 1.05, CW, "Форма от силуэта гор");
  body(s, CX, 1.45, CW, 2.0,
    "Исходная линия проекта — панорама Заилийского Алатау, которая открывается " +
    "с участка. Ломаная гряда последовательно сглаживается до плавных горизонталей " +
    "и переходит в ленты перекрытий: здание читается как продолжение ландшафта, " +
    "а не как объект, поставленный перед ним.");

  rule(s, CX, 3.55, CW);
  dashes(s, CX, 3.75, CW, 2.6, [
    "Три шага абстракции: линия хребта — сглаженный контур — ленты перекрытий.",
    "Переменная высота лент повторяет ритм гряды: объём нарастает к центру и опадает к краям.",
    "Пять ярусов держат горизонталь и не конкурируют с панорамой.",
  ]);

  s.addImage({ path: A("genesis.jpg"), x: IX, y: 1.05, w: IW, h: 5.07 });
  s.addText("Генезис формы: от силуэта Заилийского Алатау к лентам перекрытий.", {
  ...
  pageNo(s);
  s.addNotes(
    "Ключевой слайд архитектурного блока: форма не произвольная, она выведена из " +
    "силуэта хребта, который виден с участка.\n" +
    "Отсюда же следует отказ от высотности — здание держит горизонталь панорамы."
  );
}
