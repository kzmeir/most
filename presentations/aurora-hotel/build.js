   ...
   ========================================================= */
{
  ...
  s.addNotes(
    ...
    "два корпуса по 5 этажей и подземный этаж, общая площадь 27 521,32 м²."
  );
}
   ...
   ========================================================= */
{
  ...
  s.addImage({ path: A("plan_render.jpg"), x: 0.55, y: 1.05, w: 6.62, h: 4.97 });
  s.addText("Генеральный план в контексте квартала. Синим — граница участка, красным — границы смежных землепользований.", {
    x: 0.55, y: 6.12, w: 6.62, h: 0.24, isTextBox: true, margin: 0,
  ...
  dashes(s, tx, 2.45, tw, 2.5, [
    ...
    "К северу от участка, через внутриквартальный проезд, — действующая АЗС. На генплане от неё прочерчена зона радиусом 50 м; её граница проходит по северному краю пятна застройки.",
  ], { lead: 14, gap: 8 });

  rule(s, tx, 5.22, tw);
  figure(s, tx, 5.42, tw / 2 - 0.1, "1,4", "га", "Площадь земельного участка", { size: 24 });
  figure(s, tx + tw / 2, 5.42, tw / 2, "50", "м", "До действующей АЗС", { size: 24 });

  logo(s);
  pageNo(s);
  s.addNotes(
    "Красным на схеме — границы смежных землепользований, синим — граница участка.\n" +
    "АЗС: на чертеже генплана от неё прочерчена дуга радиусом 50 м. По замеру чертежа " +
    "ближайший угол пятна застройки лежит практически на этой линии — в пределах точности чертежа.\n" +
    "ГОТОВНОСТЬ К ВОПРОСУ: размер санитарно-защитной зоны АЗС зависит от типа станции " +
    "(отпускается ли газовое моторное топливо) и устанавливается по действующим Санитарным " +
    "правилам РК. Категорию действующей АЗС и её установленную СЗЗ нужно подтвердить до градсовета: " +
    "если зона окажется больше 50 м, потребуется проект СЗЗ с расчётом рассеивания либо " +
    "корректировка посадки северного корпуса.\n" +
   ...
   ========================================================= */
{
  ...
  body(s, CX, 1.45, CW, 0.9,
    "Показатели генерального плана дополнены расчётом коридоров номерных этажей " +
    "и подземного этажа — в исходных ТЭП они не выделены.", { size: 10, lead: 13 });

  rule(s, CX, 2.35, CW);
  dashes(s, CX, 2.55, CW, 4.0, [
    "Коридоры, лестнично-лифтовые узлы и служебные помещения этажей 2–5 рассчитаны от площади номеров при коэффициенте планировочной эффективности 0,70.",
    "12 693,32 / 0,70 = 18 133,31 м² номерных этажей; из них коридоры и узлы — 5 439,99 м².",
    "Подземный этаж принят в габаритах пятна застройки — 4 941,11 м². Уточняется на стадии эскизного проекта.",
    "Коэффициент использования территории 1,61 — 22 580,21 м² надземной части на 14 000 м².",
  ], { size: 10, lead: 13, gap: 8 });

  const rows = [
    ["Площадь земельного участка", "1,4 га (14 000 м²)", false],
    ["Площадь застройки", "4 941,11 м²", false],
    ["Подземный этаж — паркинг (расчётно)", "4 941,11 м²", false],
    ["Площадь 1-го этажа", "4 446,9 м²", false],
    ["Площадь номеров, нетто (этажи 2–5)", "12 693,32 м²", false],
    ["Коридоры и узлы этажей 2–5 (расчётно)", "5 439,99 м²", false],
    ["Надземная часть, всего", "22 580,21 м²", true],
    ["Общая площадь с подземным этажом", "27 521,32 м²", true],
    ["Этажность", "5 этажей + подземный", true],
  ];
  const cell = (t, o) => ({
    text: t,
    options: Object.assign(
      { fontFace: F, fontSize: 10, color: TXT, valign: "middle", margin: [7, 0, 7, 0],
  ...
  s.addTable(tbl, { x: IX, y: 1.05, w: IW, colW: [4.6, 3.0], rowH: 0.42 });

  s.addText("Источник: общие ТЭП генерального плана. Строки с пометкой «расчётно» — расчёт бюро.", {
    x: IX, y: 5.32, w: IW, h: 0.24, isTextBox: true, margin: 0,
    fontFace: F, fontSize: 8, color: GREY, valign: "middle",
  });

  const fw = IW / 3;
  figure(s, IX, 5.72, fw - 0.2, "35,3", "%", "Коэффициент застройки");
  figure(s, IX + fw, 5.72, fw - 0.2, "1,61", "", "Коэффициент использования территории");
  figure(s, IX + 2 * fw, 5.72, fw - 0.2, "64,7", "%", "Свободно от застройки");

  logo(s);
  pageNo(s);
  s.addNotes(
    "Из ТЭП генплана взяты без изменений: участок, застройка, 1-й этаж, площадь номеров.\n" +
    "РАСЧЁТ БЮРО: коридоры и узлы этажей 2–5 = 12 693,32 / 0,70 − 12 693,32 = 5 439,99 м² " +
    "(коэффициент планировочной эффективности номерного этажа 0,70). " +
    "Подземный этаж принят равным пятну застройки 4 941,11 м².\n" +
    "Отсюда: надземная часть 4 446,9 + 18 133,31 = 22 580,21 м²; с подземным этажом 27 521,32 м².\n" +
    "Коэффициенты: 4941,11/14000 = 35,3 %; 22580,21/14000 = 1,61; 14000−4941,11 = 9 058,89 м² = 64,7 %.\n" +
    "Если акимат спросит — обе расчётные строки помечены, точные значения дадут планировки на стадии ЭП."
  );
}
...
[
  ...
  ["restaurant.jpg", "Ресторан", "Зал первого этажа с выходом в сад", INK, INK,
   "Ресторан первого этажа: панорамное остекление, выход на открытую террасу в сад, раскрытие на хребет. Работает и на город, не только на постояльцев."],
  ["conference.jpg", "Конференц-зона", "Зал заседаний первого этажа", INK, INK,
   "Конференц-зал первого этажа: естественный свет и раскрытие в сад по одной стороне. Площадка для деловых мероприятий и городских событий — сегмент, который держит загрузку вне туристического сезона."],
  ["room.jpg", "Номерной фонд", "Номер с балконом верхнего яруса", WHITE, WHITE,
   ...
   ========================================================= */
{
  ...
  figure(s, IX + 2.6, 5.75, 4.9, "27 521,32", "м²", "Общая площадь с подземным этажом");
   ...
   ========================================================= */
{
  ...
  pageNo(s);
  s.addNotes(
    "Лист материалов фасадов. Палитра: анодированный алюминий цвета «шампань» по лентам " +
    "перекрытий, тёмно-серый камень/керамогранит по простенкам, остекление в графитовом " +
    "профиле, прозрачные закалённые ограждения.\n" +
    "Для градсовета важно: материалы долговечные и сдержанные по цвету, здание не вносит " +
    "в квартал контрастного пятна."
  );
}
   ...
   ========================================================= */
{
  ...
  figure(s, IX + 3.8, 5.75, 3.7, "19,7", "%", "Доля первого этажа в надземной площади");

  logo(s);
  pageNo(s);
  s.addNotes(
    "4 446,9 м² — из ТЭП. 4446,9 / 22 580,21 надземной = 19,7 %.\n" +
   ...
   ========================================================= */
{
  ...
  body(s, CX, 1.45, CW, 1.0,
    "Этажи со 2-го по 5-й отданы номерам: 12 693,32 м² чистой площади — 56,2 % надземной площади здания.");

  rule(s, CX, 2.6, CW);
  figure(s, CX, 2.8, CW, "12 693,32", "м²", "Площадь номеров, нетто");
  figure(s, CX, 3.9, CW, "≈ 4 533", "м²", "Средняя площадь номерного этажа, брутто");

  rule(s, CX, 5.0, CW);
  subTitle(s, CX, 5.14, CW, "Ёмкость номерного фонда");
  s.addText(
    [
      { text: "280 – 300", options: { fontFace: F, fontSize: 26, bold: true, color: ORANGE } },
  ...
  body(s, CX, 5.92, CW, 0.9,
    "12 693,32 м² чистой площади номеров при заявленной ёмкости дают в среднем 42–45 м² " +
    "на номер — формат просторного номера с балконом. Точная ёмкость определяется " +
    "планировками на стадии эскизного проекта.",
  ...
  pageNo(s);
  s.addNotes(
    "280–300 номеров — цифра заказчика. Проверка: 12 693,32 / 300 = 42,3 м²; / 280 = 45,3 м² " +
    "на номер. Это просторный номер с балконом — сходится с визуализацией.\n" +
    "12 693,32 / 22 580,21 надземной = 56,2 %. Номерные этажи брутто 18 133,31 / 4 = 4 533,33 м²."
  );
}
   ...
   ========================================================= */
{
  ...
  s.addImage({ path: A("plan_green.jpg"), x: IX, y: 1.05, w: IW, h: 5.07 });
  s.addText("Зелёная полоса между кольцевым проездом и границей участка.", {
    x: IX, y: 6.2, w: IW, h: 0.24, isTextBox: true, margin: 0,
    fontFace: F, fontSize: 8, color: GREY, valign: "middle",
  });
   ...
   ========================================================= */
{
  ...
  dashes(s, CX, 1.5, CW, 3.0, [
    "Два узла въезда: северо-восточный со стороны ул. Бухтарминской и юго-западный со стороны внутриквартального проезда. Оба вынесены от перекрёстка.",
    "Рампа в подземный паркинг — в юго-западной части, уходит под основной корпус; на въезде шлагбаум.",
    "Наземные гостевые места вынесены на западную кромку, вдоль улицы — гостевой транспорт не заезжает во двор.",
  ...
  body(s, CX, 5.02, CW, 0.85,
    "Подземный паркинг под основным корпусом и наземные гостевые места вдоль западной границы. " +
    "Итоговое количество определяется расчётом по нормативу к утверждённой ёмкости номерного фонда.",
    { size: 10, lead: 14 });
  ...
  s.addImage({ path: A("plan_transport.jpg"), x: IX, y: 1.05, w: IW, h: 5.07 });
  s.addText("Юго-западный узел: рампа в подземный паркинг под корпусом и ряд наземных гостевых мест вдоль улицы.", {
    x: IX, y: 6.2, w: IW, h: 0.24, isTextBox: true, margin: 0,
    fontFace: F, fontSize: 8, color: GREY, valign: "middle",
  });
  ...
  pageNo(s);
  s.addNotes(
    "На визуализированном генплане читаются: рампа под корпусом со шлагбаумом (подземный паркинг) " +
    "и ряд наземных гостевых мест вдоль западной кромки.\n" +
    "Вместимость подземного паркинга по чертежу не считается — подтвердить у заказчика и вписать " +
    "в плашку до показа. Это первый вопрос акимата."
  );
}
   ...
   ========================================================= */
{
  ...
  dashes(s, x1, 1.78, colW, 4.6, [
    ...
    "Плотность застройки: 35,3 % участка, коэффициент использования территории 1,61.",
  ...
  dashes(s, x2, 1.78, colW, 4.6, [
    ...
    "Санитарно-защитная зона действующей АЗС к северу: подтвердить тип станции и установленный размер СЗЗ. На генплане прочерчена зона 50 м.",
  ], { gap: 9 });
  ...
  figure(s, CX + 2 * gw, 5.85, gw - 0.3, "1,61", "", "Коэффициент использования территории", { size: 22 });
   ...
   ========================================================= */
{
  ...
  const fw = 4.0;
  figure(s, CX, 1.15, fw - 0.3, "280 – 300", "", "Номеров 4 звезды к фонду Алматы", { size: 24, lh: 0.5 });
  figure(s, CX + fw, 1.15, fw - 0.3, "170 – 210", "", "Постоянных рабочих мест в гостинице (оценка)", { size: 24, lh: 0.5 });
  figure(s, CX + 2 * fw, 1.15, fw - 0.3, "27 521", "м²", "Общая площадь здания с подземным этажом", { size: 24, lh: 0.5 });
  ...
  s.addText(
    "Численность персонала указана оценочно и подлежит уточнению после утверждения " +
  ...
  pageNo(s);
  s.addNotes(
    "ВНИМАНИЕ: 170–210 рабочих мест — расчёт от отраслевого ориентира 0,6–0,7 сотрудника на номер " +
    "для гостиниц 4 звезды, применённого к 280–300 номерам. Это оценка, не факт.\n" +
