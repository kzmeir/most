...
function showFile(col, rec) {
  ...
  modal((FILE_TITLE[col] || (r => r.id))(rec), `<p class="muted" style="margin-top:0">${esc(f.name)} · ${Math.max(1, Math.round((f.size || 0) / 1024))} КБ · загружен ${fdate(f.uploadedAt)} · <a href="${url}" target="_blank" rel="noopener">открыть в новой вкладке</a>${isAdmin() ? ` · <button class="btn sm danger" data-act="fileDel" data-col="${col}" data-id="${esc(rec.id)}">удалить файл</button>` : ''}</p>${/pdf/.test(f.type || '') ? `<iframe class="preview-frame" src="${url}" title="${esc(f.name)}"></iframe>` : `<img class="preview-img" src="${url}" alt="${esc(f.name)}">`}`);
  $('#modal .modal-card').style.maxWidth = '1000px'; bind($('#modal-body'));
}
const fileCell = (col, r) => r.file ? `<button class="file-chip" data-act="fileShow" data-col="${col}" data-id="${esc(r.id)}" title="${esc(r.file.name)}">${fileIcon(r.file)} ${esc(cut(r.file.name, 20))}</button>${isAdmin() ? ` <button class="btn sm" data-act="fileAttach" data-col="${col}" data-id="${esc(r.id)}" title="Заменить файл">📎</button>` : ''}` : (isAdmin() ? `<button class="btn sm" data-act="fileAttach" data-col="${col}" data-id="${esc(r.id)}" title="Прикрепить файл (PDF или фото)">📎 файл</button>` : '<span class="faint">—</span>');
...
SECTIONS.contracts = async main => {
  ...
  main.innerHTML = head('Договоры с заказчиками', `${list.length} договоров · сумма <b class="num">${money(tot('sum'))}</b> · оплачено <b class="num">${money(tot('paid'))}</b> · остаток <b class="num">${money(tot('remaining'))}</b>`, isAdmin() ? '<button class="btn primary" data-act="add">+ Договор</button>' : '') +
    ...
    (list.length ? tbl([{ t: 'Проект / договор' }, { t: 'Заказчик' }, { t: 'Исполнитель' }, { t: 'Сумма', cls: 'amt' }, { t: 'Оплачено', cls: 'amt' }, { t: 'Остаток', cls: 'amt' }, { t: 'На оплату', cls: 'amt' }, { t: 'Закрыто актами', cls: 'amt' }, { t: 'Подписан' }, { t: 'Файл' }, { t: '' }],
      list.map(c => `<tr><td><b>${esc(c.name || c.code)}</b>${c.code && c.code !== c.name ? ` <span class="faint">${esc(c.code)}</span>` : ''}<div class="faint" title="${esc(c.title)}">${esc(cut(c.title, 60))}${c.dept ? ' · ' + esc(c.dept) : ''}</div>${c.note ? `<div class="faint">${esc(cut(c.note, 60))}</div>` : ''}</td><td>${esc(c.clientShort || '')}<div class="faint">${esc(cut(c.client, 40))}</div></td><td class="muted">${esc((c.company || '').replace('ТОО ', ''))}</td><td class="amt num">${money0(c.sum)}</td><td class="amt num">${money0(c.paid)}</td><td class="amt num">${money0(c.remaining)}</td><td class="amt num">${money0(c.toPay)}</td><td class="amt num">${money0(c.closedActs)}</td><td>${yn(c.signed)}${c.closed ? ' <span class="chip neutral">закрыт</span>' : ''}</td><td>${fileCell('contracts', c)}</td><td>${actions('contracts', c.id)}</td></tr>`),
      `<tr><td colspan="3">Итого (${list.length})</td><td class="amt num">${money(tot('sum'))}</td><td class="amt num">${money(tot('paid'))}</td><td class="amt num">${money(tot('remaining'))}</td><td class="amt num">${money(tot('toPay'))}</td><td class="amt num">${money(tot('closedActs'))}</td><td colspan="3"></td></tr>`) : empty('Договоров нет'));
...
SECTIONS.proposals = async main => {
  ...
  main.innerHTML = head('Коммерческие предложения', `${rows.length} КП · сумма <b class="num">${money(rows.reduce((s, r) => s + Number(r.sum || 0), 0))}</b>`, '<button class="btn primary" data-act="add">+ КП</button>') +
    (rows.length ? tbl([{ t: '№' }, { t: 'Название' }, { t: 'Заказчик' }, { t: 'Сумма', cls: 'amt' }, { t: 'Статус' }, { t: 'Контакт с клиентом' }, { t: 'Файл' }, { t: '' }],
      rows.map(r => `<tr><td>${esc(r.no || '')}</td><td><b>${esc(r.title)}</b></td><td>${esc(r.client || '')}</td><td class="amt num">${money0(r.sum)}</td><td>${chip(r.status, { 'У клиента': ['warn', 'У клиента'], 'Выполнено': ['ok', 'Выполнено'], 'Отказ': ['crit', 'Отказ'], 'В работе': ['accent', 'В работе'] })}</td><td class="muted">${esc(r.contact || '')}</td><td>${fileCell('proposals', r)}</td><td>${actions('proposals', r.id)}</td></tr>`)) : empty('КП нет'));
