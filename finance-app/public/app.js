...
function renderNoAccess(msg) {
  $('#app').innerHTML = `<div class="auth"><div class="auth-card"><div class="brand"><div class="logo">M</div><b>MOST Финансы</b></div><h1>Нет доступа</h1><p class="sub">${esc(msg || 'Откройте страницу под рабочим аккаунтом claude.ai.')}</p><p class="faint">Доступ выдаёт владелец через меню «Поделиться» на этой странице.</p></div></div>`;
}
...
SECTIONS.dashboard = async main => {
  ...
  <div class="grid g4">
    ...
    <div class="card"><div class="label">Год: доход</div><div class="val num">${money(d.year.income)}</div><div class="foot">${Object.entries(d.year.byCompany).map(([k, v]) => `${esc(k)}: ${money(v.income)}`).join(' · ') || '—'}</div></div>
...
SECTIONS.ops = async main => {
  const u = params();
  const q = { year: u.get('year') || (u.get('ym') ? '' : today().slice(0, 4)), ym: u.get('ym') || '', account: u.get('account') || '', category: u.get('category') || '', project: u.get('project') || '', kind: u.get('kind') || '', text: u.get('text') || '', untagged: u.get('untagged') === '1' };
  const [months, res] = await Promise.all([api('/ops/months'), api('/ops?' + qsOf(q))]);
  const list = res.items, t = res.totals;
  ...
  main.innerHTML = head('Операции', `${res.total} операций${res.total > list.length ? `, показаны первые ${list.length}` : ''} · доход <b class="num">${money(t.income)}</b> · расход <b class="num">${money(t.expense)}</b> · остаток <b class="num">${money(t.net)}</b>${t.transfersIn || t.transfersOut ? ` · переводы между счетами ${money(t.transfersIn)} / ${money(t.transfersOut)}` : ''}`, '<button class="btn primary" data-act="opAdd">+ Операция</button>') +
    `<div class="filters">${sel('f-year', years, q.year, 'Все годы')}${sel('f-ym', yearMonths, q.ym, 'Все месяцы')}${sel('f-acc', accountOptions(), q.account, 'Все счета')}${sel('f-cat', categoryOptions().slice(1), q.category, 'Все категории')}${sel('f-proj', projectOptions().slice(1), q.project, 'Все проекты')}${sel('f-kind', Object.entries(KIND_LABEL), q.kind, 'Все виды')}<input id="f-text" placeholder="Поиск: контрагент, назначение" value="${esc(q.text)}" style="min-width:220px"><label class="chip ${q.untagged ? 'warn' : 'neutral'}" style="cursor:pointer"><input type="checkbox" id="f-untagged"${q.untagged ? ' checked' : ''} style="width:auto;margin-right:4px">не разнесено / авто (${t.untagged + t.auto})</label></div>` +
    (list.length ? tbl([{ t: 'Дата' }, { t: 'Счёт' }, { t: 'Контрагент' }, { t: 'Назначение' }, { t: 'Проект' }, { t: 'Категория' }, { t: 'Дебет', cls: 'amt' }, { t: 'Кредит', cls: 'amt' }, { t: '' }],
      list.map(o => `<tr data-id="${esc(o.id)}" class="${catKind(o.category) === 'transfer' ? 'row-transfer' : ''}${(!o.category || !o.project) ? ' row-untagged' : ''}"><td class="nowrap">${fdate(o.date)}</td><td class="nowrap"><span title="${esc(accCompany(o.account))}">${esc(o.account)}</span></td><td><b>${esc(cut(o.counterparty, 40))}</b></td><td class="muted" title="${esc(o.purpose)}">${esc(cut(o.purpose, 60))}${o.comment ? `<div class="faint">${esc(cut(o.comment, 60))}</div>` : ''}</td><td>${isAdmin() ? `<button class="cell-edit${o.autoProject ? ' auto' : ''}${o.project ? '' : ' none'}" data-act="cell" data-id="${esc(o.id)}" data-field="project" title="${o.autoProject ? 'Автоподсказка — нажмите, чтобы подтвердить или изменить' : 'Изменить проект'}">${esc(o.project || '—')}${o.autoProject ? ' <i>авто</i>' : ''}</button>` : esc(o.project || '—')}</td><td>${isAdmin() ? `<button class="cell-edit${o.auto ? ' auto' : ''}${o.category ? '' : ' none'}" data-act="cell" data-id="${esc(o.id)}" data-field="category" title="${o.auto ? 'Автоподсказка — нажмите, чтобы подтвердить или изменить' : 'Изменить категорию'}">${esc(o.category || '—')}${o.auto ? ' <i>авто</i>' : ''}</button>` : esc(o.category || '—')}</td><td class="amt num">${o.debit ? money(o.debit) : ''}</td><td class="amt num ok-text">${o.credit ? money(o.credit) : ''}</td><td>${actions('ops', o.id, (o.auto || o.autoProject) ? `<button class="btn sm primary" data-act="confirm" data-id="${esc(o.id)}" title="Подтвердить автоподсказку">✓</button>` : '')}</td></tr>`),
      `<tr><td colspan="6">Итого (${list.length})</td><td class="amt num">${money(list.reduce((s, o) => s + Number(o.debit || 0), 0))}</td><td class="amt num">${money(list.reduce((s, o) => s + Number(o.credit || 0), 0))}</td><td></td></tr>`) : empty('Операций по этому фильтру нет'));
  const nav = () => { const ymv = $('#f-ym').value; location.hash = '#/ops?' + qsOf({ year: ymv ? '' : $('#f-year').value, ym: ymv, account: $('#f-acc').value, category: $('#f-cat').value, project: $('#f-proj').value, kind: $('#f-kind').value, text: $('#f-text').value, untagged: $('#f-untagged').checked ? '1' : '' }); };
  ['#f-year', '#f-ym', '#f-acc', '#f-cat', '#f-proj', '#f-kind', '#f-untagged'].forEach(s => $(s).onchange = nav);
  $('#f-year').onchange = () => { $('#f-ym').value = ''; nav(); };
  $('#f-text').onkeydown = e => { if (e.key === 'Enter') nav(); };
...
SECTIONS.payroll = async main => {
  ...
  main.innerHTML = head('ЗП / ФОТ', `На руки по месяцам; с налогами — ×${(1 + tax).toFixed(2)}`, '<button class="btn primary" data-act="add">+ Месяц</button>') +
    (rows.length ? tbl([{ t: 'Месяц' }, { t: 'MOST Project', cls: 'amt' }, { t: 'MOST Architects', cls: 'amt' }, { t: 'Итого на руки', cls: 'amt' }, { t: 'С налогами', cls: 'amt' }, { t: '' }],
      rows.map(r => { const s = Number(r.project || 0) + Number(r.architects || 0); return `<tr><td><b>${esc(mname(r.id))}</b></td><td class="amt num">${money(r.project)}</td><td class="amt num">${money(r.architects)}</td><td class="amt num">${money(s)}</td><td class="amt num">${money(s * (1 + tax))}</td><td>${actions('payroll', r.id)}</td></tr>`; }),
  ...
  ACT.edit = ({ id }) => { const r = rows.find(x => x.id === id); modal('ЗП за ' + mname(id), formHtml(fields.slice(1), r), async v => { await api('/payroll/' + id, { method: 'PUT', body: numify(v, ['project', 'architects']) }); toast('Сохранено'); route(); }); };
  bind(main);
};
...
SECTIONS.settings = async main => {
  ...
  const accFields = [{ name: 'id', label: 'Код (как в операциях)', required: true, half: true }, { name: 'name', label: 'Название', half: true }, { name: 'company', label: 'Компания', type: 'select', options: [...OUR(), 'Наличные'], half: true }, { name: 'active', label: 'Активен', type: 'select', options: YESNO, half: true }];
