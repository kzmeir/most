...
async function uploadInvoiceFile(id, file) {
  if (file.size > 20 * 1024 * 1024) throw new Error('Файл больше 20 МБ');
  return api(`/invoices/${encodeURIComponent(id)}/file`, { method: 'POST', body: { name: file.name, type: guessType(file), data: await readAsBase64(file) } });
}
function showInvoiceFile(inv) {
  const f = inv.file; if (!f) return; const url = fileUrl(f);
  modal(`${inv.contractor} — ${money(inv.amount)}`, `<p class="muted" style="margin-top:0">${esc(f.name)} · ${Math.max(1, Math.round((f.size || 0) / 1024))} КБ · загружен ${fdate(f.uploadedAt)} · <a href="${url}" target="_blank" rel="noopener">открыть в новой вкладке</a>${isAdmin() ? ` · <button class="btn sm danger" data-act="invFileDel" data-id="${esc(inv.id)}">удалить файл</button>` : ''}</p>${/pdf/.test(f.type || '') ? `<iframe class="preview-frame" src="${url}" title="${esc(f.name)}"></iframe>` : `<img class="preview-img" src="${url}" alt="${esc(f.name)}">`}`);
...
SECTIONS.invoices = async main => {
  ...
  main.innerHTML = head('Счета к оплате', `Открыто: ${Object.entries(byCo).map(([k, v]) => `<b>${esc(k)}</b> ${money(v)}`).join(' · ') || 'нет'}${isAdmin() ? ' · нажмите на строку со значком файла, чтобы открыть счёт' : ''}`, isAdmin() ? '<button class="btn" data-act="invImport">⇪ Загрузить из файла</button> <button class="btn primary" data-act="invAdd">+ Счёт</button>' : '') +
    ...
    (list.length ? tbl([{ t: 'Дата' }, { t: 'Компания' }, { t: 'Контрагент' }, { t: 'Назначение' }, { t: 'Проект' }, { t: 'Сумма', cls: 'amt' }, { t: 'Статус' }, { t: 'Файл' }, { t: '' }],
      list.map(i => `<tr class="${i.file ? 'row-click' : ''}" data-id="${esc(i.id)}"><td class="nowrap">${fdate(i.invoiceDate)}${i.number ? `<div class="faint">№ ${esc(i.number)}</div>` : ''}</td><td>${esc(i.company)}</td><td><b>${esc(i.contractor)}</b></td><td class="muted" title="${esc(i.purpose)}">${esc(cut(i.purpose, 70))}</td><td class="muted">${esc(i.project)}</td><td class="amt num">${money(i.amount)}</td><td>${chip(i.status, INV_ST)}${i.paidAt ? `<div class="faint">${fdate(i.paidAt)}</div>` : ''}</td><td>${i.file ? `<button class="file-chip" data-act="invFile" data-id="${esc(i.id)}" title="${esc(i.file.name)}">${fileIcon(i.file)} ${esc(cut(i.file.name, 22))}</button>` : (isAdmin() ? `<button class="btn sm" data-act="invAttach" data-id="${esc(i.id)}" title="Прикрепить файл счёта (PDF или фото)">📎 файл</button>` : '<span class="faint">—</span>')}</td><td>${isAdmin() ? `<div class="actions">${i.status !== 'paid' ? `<button class="btn sm primary" data-act="invStatus" data-id="${i.id}" data-status="paid">Оплачен</button>` : ''}${i.status === 'open' ? `<button class="btn sm" data-act="invStatus" data-id="${i.id}" data-status="held">Придержать</button>` : ''}${i.status === 'held' ? `<button class="btn sm" data-act="invStatus" data-id="${i.id}" data-status="open">К оплате</button>` : ''}${i.file ? `<button class="btn sm" data-act="invAttach" data-id="${esc(i.id)}" title="Заменить файл">📎</button>` : ''}<button class="btn sm" data-act="edit" data-id="${i.id}">✎</button><button class="btn sm danger" data-act="del" data-col="invoices" data-id="${i.id}">✕</button></div>` : ''}</td></tr>`),
  ...
  main.querySelectorAll('tr.row-click').forEach(tr => tr.onclick = () => { const i = inv.find(x => x.id === tr.dataset.id); if (i && i.file) showInvoiceFile(i); });
  const fields = [{ name: 'company', label: 'Компания', type: 'select', options: COMPANIES(), half: true }, { name: 'invoiceDate', label: 'Дата счёта', type: 'date', value: today(), half: true }, { name: 'contractor', label: 'Контрагент', required: true, half: true }, { name: 'amount', label: 'Сумма, ₸', type: 'number', step: '0.01', required: true, half: true }, { name: 'number', label: '№ счёта', half: true }, { name: 'status', label: 'Статус', type: 'select', options: [['open', 'К оплате'], ['held', 'Придержать'], ['paid', 'Оплачен']], half: true }, { name: 'purpose', label: 'Назначение' }, { name: 'project', label: 'Проект', type: 'select', options: projectOptions(), half: true }, { name: 'category', label: 'Категория расхода (для операции)', type: 'select', options: categoryOptions(), half: true }];
  ...
  ACT.invFile = ({ id }) => { const i = inv.find(x => x.id === id); if (i) showInvoiceFile(i); };
  ACT.invFileDel = async ({ id }) => { if (!confirm('Удалить файл счёта?')) return; await api(`/invoices/${encodeURIComponent(id)}/file`, { method: 'DELETE' }); closeModal(); toast('Файл удалён'); route(); };
  ACT.invAttach = ({ id }) => { const inp = document.createElement('input'); inp.type = 'file'; inp.accept = FILE_ACCEPT; inp.id = 'inv-attach'; inp.hidden = true; document.body.appendChild(inp); inp.onchange = async () => { const f = inp.files[0]; inp.remove(); if (!f) return; try { toast('Загружаю файл…', 8000); await uploadInvoiceFile(id, f); toast('Файл прикреплён'); route(); } catch (er) { toast(er.message, 5000); } }; inp.click(); };
  ...
  $('#inv-files').onchange = async e => {
    ...
    go.onclick = async () => {
      ...
      try {
        ...
        for (const c of r.items || []) { const src = rows[c.ref]; if (src && src._file) { try { await uploadInvoiceFile(c.id, src._file); filesOk++; } catch (er) { filesErr++; console.error(er); } } }
